...
func GetUser(w http.ResponseWriter, r *http.Request) {
	...
	err := database.DB.QueryRow("SELECT users.user_id, users.name, users.email, users.is_verified, records.quiz_completed, records.avg_score, records.rank, records.points FROM users JOIN records WHERE users.user_id = $1", userId).Scan(&user.UserId, &user.Name, &user.Email, &user.IsVerified, &user.QuizCompleted, &user.AverageScore, &user.Rank, &user.TotalPoints)
	if err != nil {
